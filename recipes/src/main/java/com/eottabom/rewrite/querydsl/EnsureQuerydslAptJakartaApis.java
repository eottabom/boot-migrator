...
/**
 * querydsl-apt(jakarta) 를 쓰는 dependencies 블록에 아래 annotationProcessor 가 없으면 apt 선언 바로 위에 추가한다.
 * <pre>
 *   annotationProcessor "jakarta.annotation:jakarta.annotation-api"
 *   annotationProcessor "jakarta.persistence:jakarta.persistence-api"
 * </pre>
 * upstream {@code RemoveJakartaAnnotationDependencyWhenManagedBySpringBoot} 가 javax 코드 기준으로 판단해서
 * annotationProcessor 의 jakarta.annotation-api 를 지우는 경우가 있다. {@code org.openrewrite.gradle.AddDependency} 는
 * 같은 실행 안에서 갱신되지 않은 Gradle 모델을 보고 "이미 있다" 고 판단해서 복구하지 못하므로, LST 를 직접 확인한다.
 */
        ...
        return Preconditions.check(new IsBuildGradle<>(), new GroovyIsoVisitor<ExecutionContext>() {
            ...
            public J.Block visitBlock(J.Block block, ExecutionContext ctx) {
                ...
                for (int i = 0; i < statements.size(); i++) {
                    ...
                    if (apt == null && notation.startsWith("com.querydsl:querydsl-apt:") && notation.endsWith(":jakarta")) {
                        apt = m;
                        aptIndex = i;
                    }
