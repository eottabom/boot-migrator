 ...
 * {@code @ExtendWith(MockitoExtension.class)} 와 {@code @MockitoSettings(strictness = Strictness.LENIENT)} 를 붙인다.
 * <p>
 ...
 * MockitoExtension 만 붙이면 기본값인 strict stubs 때문에 기존에 통과하던 테스트가 UnnecessaryStubbingException 으로
 * 깨지므로, 기존 동작과 같도록 LENIENT 를 함께 지정한다. (이미 @ExtendWith 가 있는 테스트는 건드리지 않는다)
 */
public class AddLenientMockitoExtension extends Recipe {

    private static final AnnotationMatcher EXTEND_WITH = new AnnotationMatcher("@org.junit.jupiter.api.extension.ExtendWith");
    private static final AnnotationMatcher[] MOCKITO_FIELD_ANNOTATIONS = {
            new AnnotationMatcher("@org.mockito.Mock"),
            new AnnotationMatcher("@org.mockito.Spy"),
            new AnnotationMatcher("@org.mockito.InjectMocks"),
    ...
    public TreeVisitor<?, ExecutionContext> getVisitor() {
        return Preconditions.check(
                Preconditions.and(new UsesType<>("org.mockito.*", false), new UsesType<>("org.junit.jupiter.api.*", false)),
