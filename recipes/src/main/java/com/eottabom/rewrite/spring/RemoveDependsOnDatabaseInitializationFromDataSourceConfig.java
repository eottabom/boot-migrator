 ...
 * <p>
 * upstream {@code DatabaseComponentAndBeanInitializationOrdering} (Boot 2.5 단계, 이후 모든 단계에서 체이닝됨)은
 * "반환 타입에 DataSource 타입 필드/메서드가 있으면" 이 어노테이션을 붙인다. {@code HikariConfig} 는 setDataSource() 가 있어서 걸리는데,
 * 실제로는 DataSource 를 만드는 쪽이라 DB 초기화 빈 → DataSource → HikariConfig → DB 초기화 빈 순환 참조가 생긴다.
 * (BeanCurrentlyInCreationException: 'dataSourceScriptDatabaseInitializer')
 */
public class RemoveDependsOnDatabaseInitializationFromDataSourceConfig extends Recipe {
    ...
    private static final AnnotationMatcher BEAN_MATCHER = new AnnotationMatcher("@org.springframework.context.annotation.Bean");

    /** DataSource 를 만드는 데 쓰이는 설정 타입 */
    private static final List<String> DATA_SOURCE_CONFIG_TYPES = Arrays.asList(
        ...
        return Preconditions.check(new UsesType<>(DEPENDS_ON, false), new JavaIsoVisitor<ExecutionContext>() {
            ...
            public J.MethodDeclaration visitMethodDeclaration(J.MethodDeclaration method, ExecutionContext ctx) {
                ...
                if (m.getLeadingAnnotations().stream().noneMatch(BEAN_MATCHER::matches)
                    ...
                    || !isDataSourceConfig(m.getReturnTypeExpression() == null ? null : m.getReturnTypeExpression().getType())) {
                    return m;
                }
                m = (J.MethodDeclaration) new RemoveAnnotationVisitor(DEPENDS_ON_MATCHER).visitNonNull(m, ctx, getCursor().getParentOrThrow());
