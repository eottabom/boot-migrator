 ...
 * <p>
 * upstream {@code org.openrewrite.gradle.RemoveRedundantDependencyVersions} 는 groupPattern 을 줘도
 * "transitive 로 충족되는 direct 의존성" 을 통째로 지우는 로직이 함께 돌아서
 * (ex. testAnnotationProcessor lombok, spring-boot-starter-cache 삭제) 쓸 수 없다.
 */
    ...
    public TreeVisitor<?, ExecutionContext> getVisitor() {
        ...
        return Preconditions.check(new IsBuildGradle<>(), new JavaIsoVisitor<ExecutionContext>() {
            ...
            public J.MethodInvocation visitMethodInvocation(J.MethodInvocation method, ExecutionContext ctx) {
                ...
                // buildscript 의 classpath(플러그인)는 BOM 이 관리하지 않는다 (ex. spring-boot-gradle-plugin)
                if ("classpath".equals(m.getSimpleName())
                    || m.getArguments().isEmpty() || !(m.getArguments().get(0) instanceof J.Literal)) {
                    return m;
                }
                ...
                if (parts.length != 3 || !groupId.equals(parts[0]) || parts[2].isEmpty() || isBom(parts[1])) {
                    return m;
                }
