 ...
 * <p>
 * Boot 2.7 시절엔 spring-cloud 스타터 등을 타고 transitive 로 들어오던 라이브러리(commons-lang3 등)가 버전을 올리며
 * 의존성 그래프가 바뀌면 사라져 컴파일이 깨진다.
 * <p>
...
public class DeclareUsedDependency extends ScanningRecipe<DeclareUsedDependency.Accumulator> {
	...
	public DeclareUsedDependency(String packageName, String groupId, String artifactId, String version,
		...
		return new TreeVisitor<Tree, ExecutionContext>() {
			...
			public Tree visit(Tree tree, ExecutionContext ctx) {
				...
				for (J.Import imp : cu.getImports()) {
					// "org.apache.commons.lang" 이 "org.apache.commons.lang3" 에 매칭되지 않도록
					// '.' 까지 비교
	...
	public TreeVisitor<?, ExecutionContext> getVisitor(Accumulator acc) {
		return Preconditions.check(new IsBuildGradle<>(), new JavaIsoVisitor<ExecutionContext>() {
			...
			public J visit(Tree tree, ExecutionContext ctx) {
				...
				Set<String> using = (project != null) ? acc.sourceSetsUsing().get(project) : null;
				if (gp == null || using == null) {
					return (J) tree;
				}

				// 소스셋마다 자기 configuration 에 넣는다 (main -> implementation, test ->
				// testImplementation,
				// testFixtures -> testFixturesImplementation ...). main 에 넣으면 test 는
				// 따라오지만 testFixtures 는 아니다.
				J result = (J) tree;
	...
	private boolean isDeclared(GradleProject gp, List<String> configurations) {
		...
		for (String name : configurations) {
			...
			if (c != null && c.findRequestedDependency(this.groupId, this.artifactId) != null) {
				return true;
			}
		}
		return false;
