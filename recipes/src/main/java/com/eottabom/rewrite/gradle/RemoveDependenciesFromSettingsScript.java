 ...
 * <p>
 * upstream
 * {@code org.openrewrite.java.dependencies.UpgradeDependencyVersion (artifactId: *)} 이 중첩
 * 빌드(자기 settings 를 가진 하위 디렉토리)의 settings 스크립트를 빌드 파일로 보고 {@code dependencies {
 * runtimeOnly(...) }} 를 넣는 경우가 있다. settings 스크립트에는 최상위 dependencies 가 있을 수 없어(Gradle 이
 * 해석하지 못한다) 그 빌드가 깨진다. buildscript / pluginManagement 안의 dependencies 는 건드리지 않는다.
 */
		...
		return Preconditions.check(new FindSourceFiles("**/settings.gradle{,.kts}").getVisitor(),
				new JavaIsoVisitor<ExecutionContext>() {
					...
					private Statement removeIfDependencies(Statement s) {
						return (s instanceof J.MethodInvocation
								&& "dependencies".equals(((J.MethodInvocation) s).getSimpleName())) ? null : s;
					}
				});
	}
