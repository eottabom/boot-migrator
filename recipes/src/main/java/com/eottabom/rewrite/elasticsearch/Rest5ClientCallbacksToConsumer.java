 ...
 * {@code setHttpClientConfigCallback} / {@code setRequestConfigCallback} 은 {@code Consumer} 를 받는다.
 * <pre>
 ...
 * </pre>
 * 블록 마지막의 return 만 바꾼다. 중간에서 return 하는 복잡한 콜백은 그대로 두어 컴파일 에러로 드러나게 한다.
 * 식 람다({@code b -> b.setX(..)})와 메서드 참조는 Consumer 에 그대로 맞으므로 바꾸지 않는다.
 */
public class Rest5ClientCallbacksToConsumer extends Recipe {

    private static final List<String> BUILDERS = Arrays.asList(
            "org.elasticsearch.client.RestClientBuilder",
            "co.elastic.clients.transport.rest5_client.low_level.Rest5ClientBuilder");
    private static final List<String> CALLBACKS = Arrays.asList("setHttpClientConfigCallback", "setRequestConfigCallback");
