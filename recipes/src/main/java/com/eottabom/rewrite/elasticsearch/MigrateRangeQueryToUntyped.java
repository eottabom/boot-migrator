...
/**
 * elasticsearch-java 8.15 (Boot 3.4 BOM) 에서 RangeQuery 가 untyped / date / number / term 중
 * 하나를 고르는 구조로 바뀌었다. 8.14 까지 RangeQuery.Builder 에 있던 field / gte / lte 등은
 * UntypedRangeQuery.Builder 로 옮겨졌다. <pre>
 ...
 * </pre> 원래 타입(8.14 이하)의 RangeQuery.Builder 에 field 메서드가 있을 때만 바꾼다. 이미 8.15 API 로 쓴 코드는
 * 건드리지 않는다. {@code q.range(r -> r.field(...))} 처럼 람다로 쓰는 코드는 바꾸지 않는다 (컴파일 에러로 드러난다).
 */
		...
		return Preconditions.check(new UsesType<>("co.elastic.clients.elasticsearch._types.query_dsl.RangeQuery", true),
				new JavaIsoVisitor<ExecutionContext>() {
					...
					public J.MethodInvocation visitMethodInvocation(J.MethodInvocation method, ExecutionContext ctx) {
						...
						if (!"build".equals(m.getSimpleName()) || m.getSelect() == null
								|| !isLegacyBuilder(m.getSelect().getType())) {
							return m;
						}
						this.changed = true;
						// build() 는 이제 UntypedRangeQuery 를 돌려주므로 RangeQuery 가 필요한 자리에 맞게
						// 감싼다
