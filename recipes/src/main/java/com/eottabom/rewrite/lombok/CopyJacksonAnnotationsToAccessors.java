 ...
 * <p>
 * Lombok 1.18.16 ~ 1.18.38 은 필드의 {@code @JsonProperty} 등을 생성한 getter/setter 에도 복사했지만 1.18.40 부터는 복사하지 않는다.
 * 그래서 {@code @JsonProperty("isShow") boolean isShow} 는 필드(isShow)와 getter isShow()(Jackson 이름 규칙으로 show)가
 * 따로 잡혀 JSON 에 {@code isShow} 와 {@code show} 가 둘 다 나간다. 이 설정으로 이전 동작을 유지한다.
 * <p>
 * Lombok 과 Jackson 어노테이션을 함께 쓰는 소스가 있을 때만 동작한다. lombok.config 가 없으면 만들고, 있으면 한 줄 추가하고,
 * 이미 이 키가 있으면(값이 false 여도) 그대로 둔다. 구조는 upstream rewrite-jackson 의 LombokJacksonizedConfig 와 같다.
 * <p>
...
public class CopyJacksonAnnotationsToAccessors extends ScanningRecipe<CopyJacksonAnnotationsToAccessors.Accumulator> {
    ...
    public TreeVisitor<?, ExecutionContext> getScanner(Accumulator acc) {
        return new TreeVisitor<Tree, ExecutionContext>() {
            ...
            public Tree visit(Tree tree, ExecutionContext ctx) {
                if (tree instanceof SourceFile && LOMBOK_CONFIG.equals(((SourceFile) tree).getSourcePath().toString())) {
    ...
    public Collection<? extends SourceFile> generate(Accumulator acc, ExecutionContext ctx) {
        ...
        return PlainTextParser.builder().build().parse(ENTRY + "\n")
