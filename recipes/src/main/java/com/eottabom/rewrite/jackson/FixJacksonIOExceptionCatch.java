...
/**
 * upstream Jackson 3 전환이 {@code catch (IOException e)} 를 {@code catch (JacksonException e)} 로 바꿀 때
 * 같은 try 안에 {@code Files.readString()} 처럼 IOException 을 던지는 호출이 있어도 바꿔서 컴파일이 깨지는 것을 보정한다.
 ...
 * IOException 을 뺀다. (exception IOException is never thrown in body of corresponding try statement)
 * Jackson 3 는 IOException 을 던지지 않아서, upstream 이 multi-catch 에 남긴 IOException 이 컴파일 에러가 된다.
 */
        ...
        return Preconditions.check(new UsesType<>(JACKSON_EXCEPTION, false), new JavaIsoVisitor<ExecutionContext>() {
            ...
            public J.Try visitTry(J.Try tryable, ExecutionContext ctx) {
                ...
                for (int i = 0; i < catches.size(); i++) {
                    ...
                    J.Identifier io = new J.Identifier(Tree.randomId(), Space.SINGLE_SPACE, Markers.EMPTY, new ArrayList<>(),
                            "IOException", JavaType.ShallowClass.build(IO_EXCEPTION), null);
                    List<JRightPadded<NameTree>> alternatives = new ArrayList<>();
                    alternatives.add(JRightPadded.<NameTree>build((NameTree) param.getTypeExpression().withPrefix(Space.EMPTY)).withAfter(Space.SINGLE_SPACE));
                    alternatives.add(JRightPadded.<NameTree>build(io));
                    J.MultiCatch multi = new J.MultiCatch(Tree.randomId(), param.getTypeExpression().getPrefix(), Markers.EMPTY, alternatives);
            ...
            private J.Try removeIOExceptionFromJacksonMultiCatch(J.Try t) {
                ...
                for (int i = 0; i < catches.size(); i++) {
                    ...
                    boolean hasJackson = multi.getAlternatives().stream().anyMatch(a -> TypeUtils.isOfClassType(a.getType(), JACKSON_EXCEPTION));
                    ...
                    if (rest.size() == 1) {
                        updated = param.withTypeExpression((org.openrewrite.java.tree.TypeTree) rest.get(0).withPrefix(multi.getPrefix()));
                    } else {
                        List<JRightPadded<NameTree>> padded = new ArrayList<>();
                        for (int k = 0; k < rest.size(); k++) {
                            JRightPadded<NameTree> p = JRightPadded.build(k == 0 ? rest.get(k).withPrefix(Space.EMPTY) : rest.get(k).withPrefix(Space.SINGLE_SPACE));
            ...
            private boolean throwsIOException(J.Block body) {
                ...
                new JavaIsoVisitor<AtomicBoolean>() {
                    ...
                    private void check(JavaType.Method m, AtomicBoolean f) {
                        // Jackson 3 메서드는 checked 예외를 던지지 않는다. upstream 은 타입 이름만 tools.jackson 으로 바꾸고
                        // Jackson 2 로 파싱된 throws(JsonProcessingException extends IOException) 정보는 그대로 남기므로 무시한다
                        if (m != null && m.getDeclaringType().getFullyQualifiedName().startsWith("tools.jackson.")) {
                            return;
                        }
                        if (m != null && m.getThrownExceptions().stream().anyMatch(e -> TypeUtils.isAssignableTo(IO_EXCEPTION, e))) {
                            f.set(true);
                        }
                    }
                }.visit(body, found);
