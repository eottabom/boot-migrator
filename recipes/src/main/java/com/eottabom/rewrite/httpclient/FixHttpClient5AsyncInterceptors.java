...
/**
 * upstream UpgradeApacheHttpClient_5 가 타입만 HttpClient 5 로 바꾸고 남겨두는 async 빌더 인터셉터 API 차이를 맞춘다.
 * (OpenSearch RestClient 의 setHttpClientConfigCallback 처럼 HttpClient 5 HttpAsyncClientBuilder 를 다루는 코드)
 * <ul>
 *   <li>{@code addInterceptorLast/First(HttpResponseInterceptor)} → {@code addResponseInterceptorLast/First},
 *       HttpRequestInterceptor 는 {@code addRequestInterceptorLast/First}</li>
 *   <li>HttpClient 5 인터셉터 람다는 인자가 3개다: {@code (response, context) -> } → {@code (response, entity, context) -> }
 *       (incompatible parameter types in lambda expression)</li>
 * </ul>
 */
    ...
    public TreeVisitor<?, ExecutionContext> getVisitor() {
        return new JavaIsoVisitor<ExecutionContext>() {
            ...
            public J.Lambda visitLambda(J.Lambda lambda, ExecutionContext ctx) {
                ...
                JavaType target = parent instanceof J.TypeCast ? ((J.TypeCast) parent).getClazz().getTree().getType() : l.getType();
                boolean interceptor = TypeUtils.isOfClassType(target, RESPONSE_INTERCEPTOR) || TypeUtils.isOfClassType(target, REQUEST_INTERCEPTOR);
                ...
                J entity = first instanceof J.Identifier
                        ? ((J.Identifier) first).withId(Tree.randomId()).withSimpleName("entity").withPrefix(Space.SINGLE_SPACE)
                        : new J.Identifier(Tree.randomId(), Space.SINGLE_SPACE, Markers.EMPTY, new ArrayList<>(), "entity", null, null);
