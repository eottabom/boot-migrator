...
/**
 * upstream UpgradeApacheHttpClient_5 (Spring Framework 6 체인, 매 단계 실행)가 Elasticsearch 저수준
 * RestClient 설정 코드까지 HttpClient 5 로 바꾸는 것을 되돌린다. org.elasticsearch.client.RestClient 는
 * HttpClient 4(httpcore 4) API 를 쓴다. (no suitable method found for
 * builder(org.apache.hc.core5.http.HttpHost))
 * <p>
 ...
 * <li>{@code new HttpHost(scheme, host, port)} →
 * {@code new HttpHost(host, port, scheme)}</li>
 * </ul>
 */
public class RevertHttpClient5ForElasticsearchRestClient extends Recipe {
	...
	private static class ExpressionFixes extends JavaVisitor<ExecutionContext> {
		...
		public J visitNewClass(J.NewClass newClass, ExecutionContext ctx) {
			...
			if (TypeUtils.isOfClassType(n.getType(), "org.apache.http.auth.UsernamePasswordCredentials")
					&& args.size() == 2 && args.get(1) instanceof J.MethodInvocation
					&& "toCharArray".equals(((J.MethodInvocation) args.get(1)).getSimpleName())
					&& ((J.MethodInvocation) args.get(1)).getSelect() != null) {
				Expression password = ((J.MethodInvocation) args.get(1)).getSelect()
					.withPrefix(args.get(1).getPrefix());
			...
			if (TypeUtils.isOfClassType(n.getType(), "org.apache.http.HttpHost") && args.size() == 3
				...
				J.NewClass reordered = n.withArguments(Arrays.asList(host.withPrefix(scheme.getPrefix()),
						port.withPrefix(host.getPrefix()), scheme.withPrefix(port.getPrefix())));
				// 생성자 타입의 파라미터 순서도 (hostname, port, scheme) 으로 되돌린다. 남겨 두면 4.0 에서
				// upstream 이 다시
				// HttpClient 5 로 바꿀 때 ReorderMethodArguments 가 (String, int, String) 패턴과
				// 맞지 않아 순서를 안 바꾼다
				...
				if (ctor != null && ctor.getParameterTypes().size() == 3 && ctor.getParameterNames().size() == 3) {
					...
					reordered = reordered.withConstructorType(
							ctor.withParameterTypes(Arrays.asList(types.get(1), types.get(2), types.get(0)))
