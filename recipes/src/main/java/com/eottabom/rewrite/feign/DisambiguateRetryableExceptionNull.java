 ...
 * {@code new RetryableException(status, message, method, cause, null, request)} 의 null 이 Date / Long 둘 다에 매칭된다.
 * (reference to RetryableException is ambiguous). Spring Cloud 2023 (Boot 3.2) 부터 발생.
 * <p>
    ...
    public TreeVisitor<?, ExecutionContext> getVisitor() {
        return Preconditions.check(new UsesType<>(RETRYABLE_EXCEPTION, false), new JavaIsoVisitor<ExecutionContext>() {
            ...
            public J.NewClass visitNewClass(J.NewClass newClass, ExecutionContext ctx) {
                ...
                // (int status, String message, HttpMethod method, Throwable cause, Date|Long retryAfter, Request request)
                if (args.size() != 6 || !isNullLiteral(args.get(4))) {
                    return n;
                }
                Expression retryAfter = args.get(4);
                return JavaTemplate.builder("(Long) null").build()
