...
class DeclareUsedDependencyTests implements RewriteTest {
	...
	void noChangeWhenDeclaredOrNotUsed() {
		rewriteRun(mavenProject("app", buildGradle("""
				...
				import org.apache.commons.lang3.StringUtils;
				class A { String s = StringUtils.trim(" a "); }
				"""))), mavenProject("other", buildGradle("""
				plugins { id 'java' }
				repositories { mavenCentral() }
	...
	void usesSourceSetSpecificConfigurationForTestFixtures() {
		rewriteRun((spec) -> spec.typeValidationOptions(org.openrewrite.test.TypeValidation.none())
				...
				mavenProject("app", buildGradle("""
					...
					return actual;
						...
						(spec) -> spec.path("src/testFixtures/java/Fixture.java")
							.markers(org.openrewrite.java.marker.JavaSourceSet.build("testFixtures",
									java.util.Collections.emptyList())))));
	}

	@Test
	void kotlinDsl() {
		rewriteRun(mavenProject("app", buildGradleKts("""
				plugins { java }
				repositories { mavenCentral() }
				dependencies {
				    implementation("org.apache.commons:commons-text:1.10.0")
