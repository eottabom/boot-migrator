...
/**
 * yml 로 정의한 레시피가 전부 로딩되고, 참조하는 upstream 레시피 이름/옵션이 유효한지 검증한다.
 * (rewrite-recipe-bom 버전을 올렸을 때 upstream 레시피 이름이 바뀌면 여기서 먼저 깨진다)
 */
class RecipeValidationTest {
    ...
    void allCustomRecipesAreValid() {
        List<Recipe> customRecipes = ENV.listRecipes().stream()
