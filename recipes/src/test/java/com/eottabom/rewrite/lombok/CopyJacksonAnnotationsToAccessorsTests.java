...
class CopyJacksonAnnotationsToAccessorsTests implements RewriteTest {
	...
	void appendsToExistingConfig() {
		rewriteRun(java(DTO), text("lombok.addLombokGeneratedAnnotation = true", """
	...
	void keepsExistingKey() {
		rewriteRun(java(DTO),
				text("lombok.copyJacksonAnnotationsToAccessors = false\n", (spec) -> spec.path("lombok.config")));
	}

	@Test
	void unignoresRootConfig() {
		rewriteRun(java(DTO),
				text("lombok.copyJacksonAnnotationsToAccessors = true\n", (spec) -> spec.path("lombok.config")),
				text("""
						.idea/
						lombok.config
						""", """
