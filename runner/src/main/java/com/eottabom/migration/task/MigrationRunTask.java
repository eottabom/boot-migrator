...
/**
 * 현재 버전의 다음 단계부터 목표까지 단계마다 rewriteRun → compile → build → 리포트 → (commit).
 * 리포트와 패치는 대상 프로젝트의 .rewrite-migration/ 에 남는다.
 */
public abstract class MigrationRunTask extends MigrationPlanTask {
    ...
    protected MigrationRequest request() {
        ...
        return new MigrationRequest(base.projectDir(), base.targetBoot(), base.targetJava(),
                normalizeGate(getGate().getOrElse("build")), getCommit().getOrElse(false), getPreview().getOrElse(false),
                base.oneShot(), base.upstreamOnly(), getAllowDirty().getOrElse(false), base.keepJavaHome(), base.skipProjectRecipes());
    }
