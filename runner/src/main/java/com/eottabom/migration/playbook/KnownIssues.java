...
public final class KnownIssues {
     ...
     */
    public record Issue(String id, Mode mode, String title, String detail, String source, String fix,
                        String stage, List<String> requires,
                        String dependency, String crosses, List<String> affected) {
    }
    ...
    public static KnownIssues load(Path file) {
        ...
        for (Object item : Yaml.list(root.get("issues"))) {
            ...
            Issue issue = new Issue(
                    Yaml.string(m.get("id")),
                    Mode.valueOf(Yaml.string(m.get("mode"))),
                    Yaml.string(m.get("title")),
                    Yaml.string(m.get("detail")),
                    Yaml.string(m.get("source")),
                    Yaml.string(m.get("fix")),
                    Yaml.string(m.get("stage")),
                    Yaml.list(m.get("requires")).stream().map(Yaml::string).toList(),
                    Yaml.string(m.get("dependency")),
                    Yaml.string(m.get("crosses")),
                    Yaml.list(m.get("affected")).stream().map(Yaml::string).toList());
            validate(issue);
            issues.add(issue);
        }
    ...
    public List<Issue> forStage(String stageKey) {
        return issues.stream().filter(i -> stageKey.equals(i.stage())).toList();
    }
