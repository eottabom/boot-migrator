...
public record MigrationPlanner(Compatibility compatibility) {
    ...
    public MigrationPlan plan(ProjectModel project, MigrationRequest request) {
        ...
        BootLine targetLine = compatibility.boot(target);
        ...
        for (String stage : bootPath) {
            gradle = ensureGradle(plan, notes, gradle, compatibility.boot(stage), stage.equals("3.0") && fromBoot2);
        ...
        if (targetJava != null && currentJava < targetJava) {
            JavaTarget java = compatibility.java(targetJava);
            if (!java.upgradesGradle() && gradle != null && Versions.compare(gradle, java.gradleMin()) < 0) {
                notes.add("Gradle " + gradle + " 는 JDK " + targetJava + " 위에서 뜨지 않는다 (" + java.gradleMin() + "+ 필요) → Gradle 단계 추가");
                plan.add(gradleStage());
                gradle = compatibility.gradleUpgradeVersion();
            }
    ...
    private String ensureGradle(List<Stage> plan, List<String> notes, String gradle, BootLine line, boolean fromBoot2) {
        ...
        if (support == GradleSupport.TOO_OLD) {
            if (fromBoot2) {
                ...
                notes.add("Boot 3.0 은 Gradle " + line.gradleRange() + " 가 필요하다. 현재 " + gradle + " → Boot 2.x 에서 먼저 Gradle 을 올린다");
                return gradle;
            }
            notes.add("Gradle " + gradle + " 는 Boot " + line.version() + " 지원 범위(" + line.gradleRange() + ") 밖 → Gradle "
                    + compatibility.gradleUpgradeVersion() + " 단계 추가");
            plan.add(gradleStage());
            return compatibility.gradleUpgradeVersion();
        }
        if (support == GradleSupport.NOT_LISTED) {
            String note = "Gradle " + gradle + " 는 Boot " + line.version() + " 공식 지원 목록(" + line.gradleRange() + ")에 없다. 동작은 하지만 문제가 생기면 이 점을 먼저 본다";
    ...
    /**
     * auto(기본): 현재 Java 를 목표 Boot 가 지원하면 유지. latest: 목표 Boot 가 지원하는 최신 LTS. 숫자: 그 버전. none: 올리지 않음.
     */
    private Integer targetJava(String option, BootLine line) {
        String value = option == null ? "auto" : option;
        switch (value) {
            case "none":
            case "auto":
                return null;
            case "latest":
                return compatibility.latestLts(line);
            default:
                if (!value.matches("\\d+")) {
                ...
                if (version < line.javaMin() || version > line.javaMax()) {
                    throw new IllegalArgumentException("Java " + version + " 는 Boot " + line.version() + " 지원 범위(" + line.javaMin() + " ~ " + line.javaMax() + ") 밖이다");
                }
                return version;
        }
    }

    private static void javaNotes(List<String> notes, String option, Integer currentJava, Integer targetJava, BootLine line) {
        ...
        if (currentJava < line.javaMin()) {
            ...
            notes.add("Java " + currentJava + " 는 Boot " + line.version() + " 가 검증한 범위(" + line.javaMin() + " ~ " + line.javaMax() + ")보다 높다");
        } else if (currentJava >= line.javaMin() && !"none".equals(option)) {
            notes.add("Java " + currentJava + " 는 Boot " + line.version() + " 지원 범위(" + line.javaMin() + " ~ " + line.javaMax() + ") 안이라 유지한다 (올리려면 --java=latest 또는 --java=21)");
        }
    }

    private Stage gradleStage() {
        return new Stage(Stage.Kind.GRADLE, "gradle" + compatibility.gradleUpgradeVersion(), compatibility.gradleUpgradeRecipe());
    }

    /**
     * 단계별 실행(기본)은 그 단계의 변경만 담은 레시피를 쓴다. 이미 지난 단계의 레시피(Boot 2.x best practice 등)가 다시 돌지 않는다.
     * --one-shot 은 한 번에 목표까지 가야 하므로 직전 단계까지 체이닝한 레시피를 쓴다.
     * --upstream-only 는 upstream 만: 단계별이면 직전 단계 체인을 뺀 upstream 단계 레시피 (3.0 은 2.x 에서 올라오는 입구라 전체 체인).
     */
    private static Stage bootStage(String version, boolean upstreamOnly, boolean oneShot) {
        ...
        String recipe;
        if (upstreamOnly) {
            recipe = oneShot || version.equals("3.0") ? upstreamFull : "com.eottabom.rewrite.spring.upstream.UpgradeSpringBootStep_" + suffix;
        } else {
            recipe = oneShot ? "com.eottabom.rewrite.spring.MigrateToSpringBoot_" + suffix : "com.eottabom.rewrite.spring.SpringBootStep_" + suffix;
        }
